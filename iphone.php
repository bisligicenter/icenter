...
try {
    // --- CONFIGURATION ---
    $seriesMap = [
        'iphone 6 series' => ['6', '6s', '6sp'],
        'iphone 7 series' => ['7', '7p'],
        'iphone 8 series' => ['8', '8p'],
        'iphone x series' => ['x', 'xr', 'xs', 'xs max'],
        'iphone 11 series' => ['11', '11pm', '11pro'],
        'iphone 12 series' => ['12', '12pm', '12pro'],
        'iphone 13 series' => ['13', '13pro', '13pm'],
        'iphone 14 series' => ['14', '14pro', '14pm'],
        'iphone 16 series' => ['16', '16e'],
    ];

    // Centralized specifications for each iPhone series
    $specificationsMap = [
        'iphone 16 series' => ['water_resistance' => 'IP68 Water Resistant', 'display_output' => 'Super Retina XDR Display', 'screen_size' => '6.1 inches', 'charging_port' => 'USB-C', 'material' => 'Titanium and Glass', 'chip' => 'A18 Pro Chip', 'camera_feature' => '48MP Main, Ultra Wide, Telephoto'],
        'iphone 14 series' => ['water_resistance' => 'IP68 Water Resistant', 'display_output' => 'Super Retina XDR Display', 'screen_size' => '6.1 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum and Glass', 'chip' => 'A16 Bionic Chip', 'camera_feature' => '12MP Main, Ultra Wide'],
        'iphone 13 series' => ['water_resistance' => 'IP68 Water Resistant', 'display_output' => 'Super Retina XDR Display', 'screen_size' => '6.1 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum and Glass', 'chip' => 'A15 Bionic Chip', 'camera_feature' => '12MP Main, Ultra Wide'],
        'iphone 12 series' => ['water_resistance' => 'IP68 Water Resistant', 'display_output' => 'Super Retina XDR Display', 'screen_size' => '6.1 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum and Glass', 'chip' => 'A14 Bionic Chip', 'camera_feature' => '12MP Main, Ultra Wide'],
        'iphone 11 series' => ['water_resistance' => 'IP68 Water Resistant', 'display_output' => 'Liquid Retina HD Display', 'screen_size' => '6.1 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum and Glass', 'chip' => 'A13 Bionic Chip', 'camera_feature' => '12MP Main, Ultra Wide'],
        'iphone x series'  => ['water_resistance' => 'IP68 Water Resistant', 'display_output' => 'Super Retina Display', 'screen_size' => '5.8 inches', 'charging_port' => 'Lightning', 'material' => 'Stainless Steel and Glass', 'chip' => 'A11 Bionic Chip', 'camera_feature' => '12MP Main, Telephoto'],
        'iphone 8 series'  => ['water_resistance' => 'IP67 Water Resistant', 'display_output' => 'Retina HD Display', 'screen_size' => '4.7 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum and Glass', 'chip' => 'A11 Bionic Chip', 'camera_feature' => '12MP Main'],
        'iphone 7 series'  => ['water_resistance' => 'IP67 Water Resistant', 'display_output' => 'Retina HD Display', 'screen_size' => '4.7 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum and Glass', 'chip' => 'A10 Fusion Chip', 'camera_feature' => '12MP Main'],
        'iphone 6 series'  => ['water_resistance' => 'Not Rated', 'display_output' => 'Retina HD Display', 'screen_size' => '4.7 inches', 'charging_port' => 'Lightning', 'material' => 'Aluminum', 'chip' => 'A9 Chip', 'camera_feature' => '12MP Main'],
    ];

    // Default specifications for unknown models
    $defaultSpecs = [
        'water_resistance' => 'Water Resistant',
        'display_output'   => 'Retina Display',
        'screen_size'      => '6.1 inches',
        'charging_port'    => 'Lightning',
        'material'         => 'Aluminum and Glass',
        'chip'             => 'Apple Chip',
        'camera_feature'   => '12MP Camera'
    ];
    ...
    $modelStmt = $conn->prepare("SELECT DISTINCT LOWER(model) AS model FROM products WHERE LOWER(product) = 'iphone' AND (archived IS NULL OR archived = 0) ORDER BY model ASC");
    $modelStmt->execute();
    $dbModels = $modelStmt->fetchAll(PDO::FETCH_COLUMN);

    // Create a combined list of filter options (series + individual models)
    $allFilterOptions = array_keys($seriesMap);
    $allModelsInSeries = array_merge(...array_values($seriesMap));
    foreach ($dbModels as $model) {
        if (!in_array(strtolower($model), $allModelsInSeries)) {
            $allFilterOptions[] = $model;
        }
    }

    // Fetch distinct storage options for iPhones that have products
    $storageStmt = $conn->prepare("SELECT DISTINCT LOWER(storage) AS storage FROM products WHERE LOWER(product) = 'iphone' AND (archived IS NULL OR archived = 0) ORDER BY storage ASC");
    $storageStmt->execute();
    $storageOptions = $storageStmt->fetchAll(PDO::FETCH_COLUMN);

    // --- BUILD AND EXECUTE QUERIES ---
    $baseQuery = "FROM products WHERE LOWER(product) = 'iphone' AND (archived IS NULL OR archived = 0)";
    $whereConditions = [];
    $queryParams = [];

    if ($modelBrandFilter !== '' && $modelBrandFilter !== 'all models') {
        if (isset($seriesMap[$modelBrandFilter])) {
            $models = $seriesMap[$modelBrandFilter];
            $placeholders = implode(',', array_fill(0, count($models), '?'));
            $whereConditions[] = "LOWER(model) IN ($placeholders)";
            $queryParams = array_merge($queryParams, $models);
        } else {
            $whereConditions[] = "LOWER(model) = ?";
            $queryParams[] = $modelBrandFilter;
        }
    }

    if ($storageFilter !== '' && $storageFilter !== 'all storages') {
        $whereConditions[] = "LOWER(storage) = ?";
        $queryParams[] = $storageFilter;
    }
    ...
    // Select query
    $selectQuery = "SELECT * " . $baseQuery . $whereClause . " ORDER BY $orderByClause LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($selectQuery);
    
    $selectParams = $queryParams;
    $selectParams[] = $itemsPerPage;
    $selectParams[] = $offset;
    $stmt->execute(array_values($selectParams));
...
<head>
  ...
  <style>
    ...
    /* Hide search bar in header for iphone page only */
    .search-container {
...
<?php include 'kioskheader.php'; ?>
  ...
  <main class="p-8">
    ...
    <div class="flex flex-wrap gap-2 mb-4 justify-center" id="modelBrandButtons">
      <?php
        ...
        // Show all available models and series
        foreach ($allFilterOptions as $model) {
            ...
            // Prettify the label
            $label = ucwords(str_replace(' series', ' Series', $model));
            
            echo '<a href="?' . $queryString . '" class="px-6 py-3 rounded-full border-2 border-blue-700 font-bold cursor-pointer transition-all duration-200 shadow-md m-1 ' . $activeClass . '" title="Filter by ' . htmlspecialchars($label) . '" aria-label="Filter by ' . htmlspecialchars($label) . '" style="user-select:none;">' . htmlspecialchars($label) . '</a>';
        }
      ?>
    </div>
    ...
    <div class="dropdown-product-grid">
      ...
      if (count($products) === 0) {
          ...
          foreach ($products as &$product) { // Use reference to modify
              // Add technical specifications based on iPhone model
              $modelLower = strtolower($product['model']);
              $brandLower = strtolower($product['brand']);

              if ($brandLower === 'apple' && strpos(strtolower($product['product']), 'iphone') !== false) {
                  $specsToApply = $defaultSpecs; // Start with default specs
                  // Find which series the model belongs to and get its specific specs
                  foreach ($seriesMap as $seriesName => $seriesModels) {
                      if (in_array($modelLower, $seriesModels)) {
                          if (isset($specificationsMap[$seriesName])) {
                              $specsToApply = $specificationsMap[$seriesName];
                          }
                          break; // Found the series, no need to check further
                      }
                  }

                  // Apply the specs, only if the field is empty in the product data
                  foreach ($specsToApply as $key => $value) {
                      $product[$key] = empty($product[$key]) ? $value : $product[$key];
                  }
              }

              // Use the enhanced renderProductCard function
  ...
  <script>
    // Function to update thumbnails in the details modal
    function updateThumbnails(images, productName, mainImage) {
      ...
      images.forEach((src, idx) => {
        if (!src) return; // Skip empty image sources
  ...
  document.addEventListener('click', function(e) {
    ...
    if (target.classList.contains('details-btn') || target.closest('.details-btn')) {
      ...
      const reserveBtn = detailsModal.querySelector('.reserve-from-details-btn');
      ...
      let images = [];
      try { images = JSON.parse(imagesJson); } catch { images = [image]; }
      if (!Array.isArray(images) || images.length === 0) images = [image];

      // Store product ID on the modal for the reserve button
      detailsModal.setAttribute('data-product-id', productId);

      if (modalName) { modalName.textContent = name; }
      if (modalDescription) { modalDescription.textContent = description; }
      if (modalImage) { modalImage.src = images[0] || image; modalImage.alt = name; }
      if (modalPrice) { modalPrice.textContent = price; }
      if (modalBrand) { modalBrand.textContent = brand; }
      if (modalModel) { modalModel.textContent = model; }
      if (modalStorage) { modalStorage.textContent = storage; }

      if (modalWaterResistance) { modalWaterResistance.textContent = waterResistance || 'Not specified'; }
      if (modalDisplayOutput) { modalDisplayOutput.textContent = displayOutput || 'Not specified'; }
      if (modalScreenSize) { modalScreenSize.textContent = screenSize || 'Not specified'; }
      if (modalChargingPort) { modalChargingPort.textContent = chargingPort || 'Not specified'; }
      if (modalMaterial) { modalMaterial.textContent = material || 'Not specified'; }
      if (modalChip) { modalChip.textContent = chip || 'Not specified'; }
      if (modalCameraFeature) { modalCameraFeature.textContent = cameraFeature || 'Not specified'; }

      const technicalSpecsSection = detailsModal.querySelector('.technical-specs');
      if (technicalSpecsSection) {
        const values = [waterResistance, displayOutput, screenSize, chargingPort, material, chip, cameraFeature];
    ...
    // Handle "Reserve Now" button inside the details modal
    if (target.classList.contains('reserve-from-details-btn')) {
        e.preventDefault();
        e.stopPropagation();

        const detailsModal = document.getElementById('detailsModal');
        const reserveModal = document.getElementById('reserveModal');
        if (!detailsModal || !reserveModal) return;
        ...
        // Get product data from the details modal
        const productId = detailsModal.getAttribute('data-product-id');
        const name = detailsModal.querySelector('.modal-product-name')?.textContent || '';
        const image = detailsModal.querySelector('.modal-product-image')?.src || '';
        const price = detailsModal.querySelector('.modal-product-price')?.textContent || '';
        const brand = detailsModal.querySelector('.modal-product-brand')?.textContent || '';
        const model = detailsModal.querySelector('.modal-product-model')?.textContent || '';

        // Populate and open the reservation modal
        const reserveImage = reserveModal.querySelector('.modal-product-image');
        const reserveName = reserveModal.querySelector('.modal-product-name');
        const reserveBrand = reserveModal.querySelector('.modal-product-brand');
        const reserveModel = reserveModal.querySelector('.modal-product-model');
        const reservePrice = reserveModal.querySelector('.modal-product-price');
        const reserveId = reserveModal.querySelector('.modal-product-id span');

        if (reserveImage) reserveImage.src = image;
        if (reserveName) reserveName.textContent = name;
        if (reserveBrand) reserveBrand.textContent = brand;
        if (reserveModel) reserveModel.textContent = model;
        if (reservePrice) reservePrice.textContent = price;
        if (reserveId) reserveId.textContent = productId;
